...
class MailSettings(Base):
    ...
    # Optional second SMTP (used if primary fails or Gmail API fails)
    smtp_backup_host: Mapped[str | None] = mapped_column(String(255), nullable=True)
    smtp_backup_port: Mapped[int | None] = mapped_column(Integer, nullable=True)
    smtp_backup_from_email: Mapped[str | None] = mapped_column(String(255), nullable=True)
    smtp_backup_use_tls: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    smtp_backup_use_ssl: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    smtp_backup_user: Mapped[str | None] = mapped_column(String(255), nullable=True)
    smtp_backup_password: Mapped[str | None] = mapped_column(String(255), nullable=True)
...
class Release(Base):
    ...
    minisite_slug: Mapped[str | None] = mapped_column(String(160), nullable=True, index=True)
    minisite_is_public: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    minisite_json: Mapped[str] = mapped_column(Text, default="{}")
    created_at: Mapped[DateTime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    ...
    link_candidates: Mapped[list["ReleaseLinkCandidate"]] = relationship(
        back_populates="release",
        order_by="desc(ReleaseLinkCandidate.discovered_at)",
        cascade="all, delete-orphan",
    )
    link_scan_runs: Mapped[list["ReleaseLinkScanRun"]] = relationship(
        back_populates="release",
        order_by="desc(ReleaseLinkScanRun.created_at)",
        cascade="all, delete-orphan",
    )


class ReleaseLinkCandidate(Base):
    __tablename__ = "release_link_candidates"
    ...
    match_title: Mapped[str | None] = mapped_column(String(300), nullable=True)
    match_artist: Mapped[str | None] = mapped_column(String(300), nullable=True)
    confidence: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    status: Mapped[str] = mapped_column(String(30), default="pending_review", nullable=False, index=True)
    source_type: Mapped[str] = mapped_column(String(30), default="web_search", nullable=False)
    raw_payload_json: Mapped[str] = mapped_column(Text, default="{}")
    discovered_at: Mapped[DateTime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    reviewed_at: Mapped[DateTime | None] = mapped_column(DateTime(timezone=True), nullable=True)

    release: Mapped["Release"] = relationship(back_populates="link_candidates")


class ReleaseLinkScanRun(Base):
    __tablename__ = "release_link_scan_runs"
    ...
    release: Mapped["Release"] = relationship(back_populates="link_scan_runs")

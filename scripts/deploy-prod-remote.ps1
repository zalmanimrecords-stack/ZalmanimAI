...
$remoteCmd = "cd $repoPathOnVps && export IMAGE_TAG=`$(date +%Y-%m-%d-%H%M) && export GIT_LAST_UPDATE=`$(date -u +'%Y-%m-%d %H:%M:%S UTC') && ( [ -f deploy/build_number ] && export BUILD_NUMBER=`$((`$(cat deploy/build_number)+1)) || export BUILD_NUMBER=1 ) && echo `$BUILD_NUMBER > deploy/build_number && echo '[deploy] Image tag:' `$IMAGE_TAG 'build:' `$BUILD_NUMBER && git pull && docker compose --env-file deploy/.env.production -f docker-compose.prod.yml build --no-cache web api worker && docker compose --env-file deploy/.env.production -f docker-compose.prod.yml up -d && docker compose --env-file deploy/.env.production -f docker-compose.prod.yml restart api worker web && docker compose --env-file deploy/.env.production -f docker-compose.prod.yml ps"

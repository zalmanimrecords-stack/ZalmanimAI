...
# Pull, rebuild web+api+worker with --no-cache (fresh Flutter + Python layers), up -d, restart app containers, show ps.
# Echo markers make it obvious which step failed (git pull vs build vs up).
$remoteCmd = "cd $repoPathOnVps && export IMAGE_TAG=`$(date +%Y-%m-%d-%H%M) && export GIT_LAST_UPDATE=`$(date -u +'%Y-%m-%d %H:%M:%S UTC') && ( [ -f deploy/build_number ] && export BUILD_NUMBER=`$((`$(cat deploy/build_number)+1)) || export BUILD_NUMBER=1 ) && echo `$BUILD_NUMBER > deploy/build_number && echo '[deploy] Image tag:' `$IMAGE_TAG 'build:' `$BUILD_NUMBER && echo '[deploy] Step: git pull' && git pull && echo '[deploy] Step: docker compose build' && docker compose --env-file deploy/.env.production -f docker-compose.prod.yml build --no-cache web api worker && echo '[deploy] Step: docker compose up -d' && docker compose --env-file deploy/.env.production -f docker-compose.prod.yml up -d && echo '[deploy] Step: docker compose restart' && docker compose --env-file deploy/.env.production -f docker-compose.prod.yml restart api worker web && echo '[deploy] Step: docker compose ps' && docker compose --env-file deploy/.env.production -f docker-compose.prod.yml ps"
